...
<body class="bg-slate-50 mt-16 text-gray-900 min-h-screen">
    ...
    <main class="ml-10 p-8 min-h-screen">
        ...
        <div class="mt-20 space-y-8">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 btn-primary rounded-3xl p-8 text-white relative overflow-hidden flex flex-col justify-between shadow-xl">
                    <div class="relative z-10">
                        <div class="mt-0 space-y-4">
                            <div class="flex justify-between items-end">
                                <div>
                                    <p class="text-blue-100 text-xs mb-1">{{ $badge->name }}</p>
                                    <p class="text-xl font-bold font-headline">{{Auth::user()->xp}}.00 XP </p>
                                </div>                         
                            </div>
                        </div>
                    </div>
                    <div class="absolute -right-20 -top-20 w-64 h-64 bg-blue-500 rounded-full blur-[100px] opacity-50">
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div class="xl:col-span-2 space-y-8">
                    <section>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            @forelse ($requests as $request)
                                <div class="bg-white p-6 rounded-2xl border border-transparent hover:shadow-md transition-shadow group">
                                    ...
                                    <div class="flex justify-between items-start mb-4">
                                        ...
                                        <span class="badge-secondary text-[10px] font-bold px-2 py-1 rounded-full uppercase">
                                            {{ $request->name }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="flex gap-2">
                                        <button data-id="{{ $request->id }}" data-teacher="{{ $request->teacher_id }}"
                                            ...
                                            class="accept-btn flex-1 py-2 btn-primary rounded-lg text-sm font-bold active:scale-95 transition-all">
                                            Accept
                                        </button>

                                        <button data-id="{{ $request->id }}" data-name="{{ $request->firstname }}"
                                            class="decline flex-1 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm font-bold hover:bg-red-100 hover:text-red-600 active:scale-95 transition-all">
                                            Decline
                                        </button>
                                    </div>
                                </div>
                            ...
                            @empty
                                ...
                                <div class="flex hidden gap-2">
                                    <button class="accept-btn flex-1 py-2 btn-primary rounded-lg text-sm font-bold active:scale-95 transition-all">
                                        Accept
                                    </button>

                                    <button class="decline flex-1 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm font-bold hover:bg-red-100 hover:text-red-600 active:scale-95 transition-all">
                                        Decline
                                    </button>
                                </div>
                            @endforelse
                                    ...
                                    <tbody class="divide-y divide-gray-100">
                                        @forelse ($ActiveSessions as $ActiveSession)
                                            <tr class="hover:bg-gray-50 transition-colors">
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center gap-3">
                                                        <div class="w-8 h-8 rounded-full bg-teal-600 flex items-center justify-center text-white text-xs font-bold">
                                                ...
                                                <td class="px-6 py-4">
                                                    <span class="flex items-center gap-2 text-xs font-bold text-teal-brand">
                                                ...
                                                <td class="px-6 py-4 text-right">
                                                    @if(!empty($ActiveSession->canJoin) && $ActiveSession->canJoin)
                                                        <a href="messages/{{ $ActiveSession->id }}"
                                                            class="btn-primary px-4 py-2 rounded-lg text-white text-xs font-bold active:scale-95 transition-all inline-block">
                                                            Enter
                                                        </a>
                                                    @else
                                                        <button disabled
                    ...
                    <section class="bg-white rounded-3xl p-8 border border-gray-200">
                        <div class="flex justify-between items-center mb-6">
                            <div>
                                <h3 class="text-xl font-extrabold font-headline">My Teaching Skills</h3>
                                <p class="text-sm text-gray-600">List of your active teaching skills with certificate request actions.</p>
                            </div>
                            <span class="text-sm text-gray-500">{{ $teacherSkills->count() }} skills</span>
                        </div>
                        <div class="space-y-4 max-h-[400px] overflow-y-auto">
                            @forelse ($teacherSkills as $skill)
                                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-4 bg-gray-50 rounded-3xl border border-gray-200">
                                    <div>
                                        <p class="font-semibold text-gray-900">{{ $skill->name }}</p>
                                        <p class="text-sm text-gray-600">Level {{ $skill->level }} · {{ $skill->years_experience ?? 0 }} years experience</p>
                                    </div>
                                    <button type="button"
                                        class="request-certificate-btn inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-semibold transition-colors btn-primary hover:opacity-90 disabled:bg-gray-300 disabled:text-gray-600 disabled:cursor-not-allowed"
                ...
                <div class="space-y-8">
                    <section class="bg-white rounded-3xl p-8 border border-gray-200">
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="font-headline font-bold text-lg text-gray-900">Student Feedback</h3>
                        </div>
                        <div class="space-y-6">
                                ...
                                <div class="space-y-3">
                                    <div class="flex justify-between items-center">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-xs font-bold text-gray-900">{{ $review->firstname . $review->lastname }}</span>
                                        </div>
    ...
    <div id="learningpopup" class="fixed hidden inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div class="bg-white p-8 rounded-lg w-full max-w-[440px] relative shadow-xl border border-slate-200">
            ...
            <form id="acceptReq" method="POST" action="{{ Route('connect.session') }}">
                ...
                <div class="mb-5">
                    <label class="block text-sm font-medium text-gray-900 mb-1">Session Type</label>
                ...
                <div class="flex flex-col gap-3">
                    <button
                        class="w-full btn-primary font-bold px-6 py-3.5 rounded-lg transition-all shadow-md active:scale-[0.98]">
                        Confirm Schedule
                    </button>
                    <button
    ...
    <div id="declinePopup" class="fixed inset-0 hidden bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div class="bg-white p-8 rounded-lg w-full max-w-[440px] relative shadow-xl border border-slate-200">
